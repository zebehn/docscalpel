"""
Equation detector module using DocLayout-YOLO.

Detects mathematical equations in PDF pages using DocLayout-YOLO.
"""
...
class EquationDetector:
    """
    Detector for mathematical equations in PDF pages.

    Uses DocLayout-YOLO model for equation detection.
    """

    def __init__(self, confidence_threshold: float = 0.5):
        """
        Initialize EquationDetector with DocLayout-YOLO model.
    ...
    def model(self):
        ...
        if not self._model_loaded:
            try:
                logger.info("Loading DocLayout-YOLO model for equation detection...")
                ...
                raise RuntimeError(
                    f"Failed to load DocLayout-YOLO model for equation detection: {e}"
                )
        return self._model

    def detect(self, page: Page, pdf_path: str = None) -> List[Element]:
        """
        Detect equations in a PDF page.

        Args:
            page: Page object to analyze
            pdf_path: Path to the PDF file for rendering

        Returns:
            List of Element objects with type EQUATION, filtered by confidence threshold
        ...
        try:
            logger.debug(
                f"Equation detection on page {page.page_number} "
                ...
                if hasattr(detections, 'boxes'):
                    ...
                    for i in range(len(boxes)):
                        ...
                        if class_name.lower() in ['equation', 'formula', 'math']:
                            ...
                            logger.debug(
                                f"Detected equation on page {page.page_number}: "
                                f"confidence={confidence:.2f}, bbox=({x1:.0f},{y1:.0f},{x2:.0f},{y2:.0f})"
                            )

            logger.debug(f"Found {len(elements)} equations on page {page.page_number}")
            return elements

        except Exception as e:
            logger.error(f"Equation detection failed on page {page.page_number}: {e}")
            return []
