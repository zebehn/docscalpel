...
- **Subfigure handling** - merges separately detected subfigures (a, b, c) that share the same caption
...
5. **Subfigure Merging**: Combines separately detected subfigures that share the same caption number
6. **Coordinate Transformation**: Precise scaling from image to PDF coordinates
7. **Smart Numbering**: Uses parsed caption numbers when available, falls back to sequential numbering
8. **Overlap Resolution**: Keeps highest-confidence elements when different types overlap
9. **Precise Extraction**: Vector-preserving PDF cropping
