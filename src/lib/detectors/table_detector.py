"""
Table detector module using DocLayout-YOLO.

Detects tables in PDF pages using DocLayout-YOLO as the primary method.
"""
...
class TableDetector:
    """
    Detector for tables in PDF pages.

    Uses DocLayout-YOLO model for table detection.
    """

    def __init__(self, confidence_threshold: float = 0.5):
        """
        Initialize TableDetector with DocLayout-YOLO model.
    ...
    def model(self):
        ...
        if not self._model_loaded:
            try:
                logger.info("Loading DocLayout-YOLO model for table detection...")
                ...
                raise RuntimeError(
                    f"Failed to load DocLayout-YOLO model for table detection: {e}"
                )
        return self._model
    ...
    def detect(self, page: Page, pdf_path: str = None) -> List[Element]:
        ...
        Returns:
            List of Element objects with type TABLE, filtered by confidence threshold
        ...
        try:
            logger.debug(
                f"Table detection on page {page.page_number} "
                ...
                if hasattr(detections, 'boxes'):
                    ...
                    for i in range(len(boxes)):
                        ...
                        if class_name.lower() in ['table']:
                            ...
                            logger.debug(
                                f"Detected table on page {page.page_number}: "
                                f"confidence={confidence:.2f}, bbox=({x1:.0f},{y1:.0f},{x2:.0f},{y2:.0f})"
                            )

            logger.debug(f"Found {len(elements)} tables on page {page.page_number}")
            return elements
